...
@Component({
  selector: 'app-alusul-rajab1417',
  templateUrl: './alusul-rajab1417.page.html',
  styleUrls: ['./alusul-rajab1417.page.scss'],
})
export class AlusulRajab1417Page implements OnInit {
...
constructor(private http: HttpClient,private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'alusul-rajab1417');
        this.storage.get('alusul-rajab1417-mp3').then(link => {
            if(link){
    ...
    getlink(link:string){
        this.storage.set('alusul-rajab1417-mp3', link);
    }
    forwardLink(){
        this.storage.get('alusul-rajab1417-mp3').then((link) => {
            var options = {
