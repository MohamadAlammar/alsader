...
@Component({
  selector: 'app-alusul-jamade-tane1417',
  templateUrl: './alusul-jamade-tane1417.page.html',
  styleUrls: ['./alusul-jamade-tane1417.page.scss'],
})
export class AlusulJamadeTane1417Page implements OnInit {
 ...
 ngOnInit() { 
  const url= 'https://strapi.alsader.net/api/aduruses?filters[adurus_cat][title][$eq]=bahes-kharej-ausul-jamade-tene-1417&populate=*'
...
constructor(private storage: Storage,public navCtrl: NavController,private streamingMedia: StreamingMedia,private router: Router,private popoverCtrl: PopoverController,private popoverController: PopoverController,public loadingController: LoadingController) {
    ...
    ionViewWillEnter(){
        ...
        this.storage.set('page-aldurus', 'alusul-jamade-tane1417');
        this.storage.get('alusul-jamade-tane1417-mp3').then(link => {
            if(link){
    ...
    getlink(link:string){
        this.storage.set('alusul-jamade-tane1417-mp3', link);
    }
    forwardLink(){
        this.storage.get('alusul-jamade-tane1417-mp3').then((link) => {
            var options = {
